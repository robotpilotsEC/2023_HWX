...
HAL_StatusTypeDef MOTOR_6020_CAN2_SENT_DATA( int16_t data_1,int16_t data_2,int16_t data_3,int16_t data_4)//晚点再改，先转起来
{
	...
	//data_1 不能用
	data_2 = Master_Head_structure.From_R_Head.measure_yaw;
	data_3 = Master_Head_structure.From_R_Head.measure_pit;
	/*数据处理*/
	data[0]      = data_1 >> 8;
	data[1]      = data_1;
	data[2]      = data_2 >> 8;
	...
	data[4]      = data_3 >> 8;
	data[5]      = data_3;
	data[6]      = Master_Head_structure.From_R_Head.gimbal_mode;
	data[7]      = Master_Head_structure.From_R_Head.shoot_mode;
	/*数据帧开头*/
	txFrameHeader.StdId = MOTOR_6020_SENT_ID;
	txFrameHeader.IDE   = CAN_ID_STD;
	txFrameHeader.RTR   = CAN_RTR_DATA;
...
void MOTOR_6020_GET1_DATA(motor_6020_t *motor)
{
	...
	if(motor->base_info->temperature>=60 || motor->base_info->temperature<=20)
	{
		return;
	}
	/*更新信息 */
