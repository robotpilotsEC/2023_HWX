...
static void uart_rx_idle_callback(UART_HandleTypeDef* huart)
{
	...
	if (huart == &huart2)
	{
		...
		if ((USART2_RX_BUF_LEN - huart->hdmarx->Instance->NDTR) == USART2_RX_DATA_FRAME_LEN)//确认接收到18u NDTR为未发送的字节数
		{
			if(huart->hdmarx->Instance->CR & DMA_SxCR_CT)//0：当前目标存储器为存储器 0（使用 DMA_SxM0AR 指针寻址）1：当前目标存储器为存储器 1（使用 DMA_SxM1AR 指针寻址）这是在判断使用哪个数据缓冲区
				huart->hdmarx->XferM1CpltCallback(huart->hdmarx);// 使用储存器1   地址1发送完成中断回调函数
			else
				huart->hdmarx->XferCpltCallback(huart->hdmarx);// 使用储存器0     传输完成回调函数
		}
                                                    ...
                                                    uint32_t DataLength)
{
	...
	{	
		...
		/* Clear TC flags */
		__HAL_DMA_CLEAR_FLAG (hdma, __HAL_DMA_GET_TC_FLAG_INDEX(hdma));//current DMA Stream transfer complete flag.
